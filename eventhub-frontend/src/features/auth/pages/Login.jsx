            ...
            <div className="w-full max-w-6xl min-h-[650px] bg-white rounded-[30px_12px_30px_12px] overflow-hidden grid lg:grid-cols-2 shadow-sm">
                ...
                <div className="flex items-center justify-center px-8 py-12">
                    <div className="w-full max-w-md">
                        ...
                        {error && (
                            <p className="mt-5 bg-red-50 text-red-500 text-sm p-3 rounded-xl">{error}</p>
                        )}

                        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
                            <div>
                                <label className="text-sm font-medium text-[#20231F]">Adresse email</label>
                                <input type="email" value={email} onChange={(e)=>setEmail(e.target.value)} placeholder="exemple@email.com" required className="mt-2 w-full h-12 border border-stone-300 rounded-[14px_6px_14px_6px] px-4 text-sm outline-none focus:border-[#66735A]"/>
                            </div>

                            <div>
                                <label className="text-sm font-medium text-[#20231F]">Mot de passe</label>
                                <input type="password" value={password} onChange={(e)=>setPassword(e.target.value)} placeholder="Votre mot de passe" required className="mt-2 w-full h-12 border border-stone-300 rounded-[14px_6px_14px_6px] px-4 text-sm outline-none focus:border-[#66735A]"/>
                            </div>

                            <button type="submit" disabled={loading} className="w-full h-12 bg-[#263128] hover:bg-[#354137] text-white rounded-[14px_6px_14px_6px] text-sm font-semibold transition disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                        ...
                        <p className="mt-7 text-center text-sm text-stone-500">
                            ...
                            <button onClick={()=>navigate("/register")} className="font-semibold text-[#66735A] hover:text-[#263128]">Créer un compte</button>
                        </p>
